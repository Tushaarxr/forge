...
**Version 0.2.3** · Local-first autonomous coding agent
...
**When to use:** Once per machine (for global settings) and once per project (for `.forge/.env`). During setup, you can configure your **API Call Budget** (Minimal, Balanced, Maximum, or Custom) which controls how often Forge sends batches for review. The **Quota Estimator** will show you how many API calls a typical 20-task run will consume based on your choice.

If you get an "API key missing" error, run `forge setup --key`.
...
**What it does:** **Fully autonomous mode.** The Gemini brain generates a complete project roadmap (15–40 tasks), then executes every task one by one. To optimize API usage, Forge performs **token-efficient batch reviews** every N tasks. It pauses for a human checkpoint where you can: continue, edit the plan, rollback, or stop. The checkpoint panel displays an **API Call Counter** to track your current usage.
...
| `--plan-file` | none | Load a pre-generated plan (JSON, file, stdin, or string) |
| `--max-tasks`, `-m` | `50` | Hard stop after N tasks (safety limit) |
| `--yes`, `-y` | off | Skip the initial confirmation prompt |

#### Deep Dive: Using Predefined Plans

You can bypass the "Master Brain" planning phase by providing your own roadmap. This is useful for repeating common workflows or using plans generated by other tools.

Forge is extremely flexible with how you provide these tasks:

*   **Method A: Local File**
    Create a file (e.g., `tasks.txt`) in your project root and point to it.
    ```bash
    forge auto --plan-file tasks.txt
    ```
*   **Method B: Standard Input (Piping)**
    Pipe a plan directly into Forge using the `-` shortcut.
    ```bash
    cat my_roadmap.md | forge auto --plan-file -
    ```
*   **Method C: Raw String**
    Pass the tasks directly as a string. If the string contains newlines or doesn't match an existing file, Forge treats it as a plan.
    ```bash
    forge auto --plan-file "1. Create models.py\n2. Add migrations\n3. Test"
    ```

**Supported Formats:**
Forge's parser is very forgiving. It supports:
- **JSON**: A standard Forge `plan.json` export.
- **Numbered Lists**: `1. Task A`, `2. Task B`
- **Bullet Lists**: `- Task A`, `* Task B`
- **Bare Lines**: Simply one task per line.

**Examples:**
```bash
forge auto "build a FastAPI app"          # normal mode (Gemini plans)
forge auto --plan-file roadmap.txt        # file mode
cat plan.json | forge auto --plan-file -  # piping mode
```
...
🧠 Master Brain       ✅ gemini/gemini-2.5-flash
```
...
forge auto --plan-file roadmap.txt   # start from a text-based plan
```
